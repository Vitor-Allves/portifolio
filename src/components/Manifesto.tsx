...
export default function Manifesto() {
  return (
    <section className="relative bg-ice-100 text-navy-950 py-32 sm:py-44 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-3xl px-6 lg:px-10 flex flex-col items-center gap-7 text-center">
        {lines.map((line, i) => (
          <Reveal key={line} delay={i * 0.18} y={16}>
            <p className="font-serif text-xl sm:text-2xl lg:text-3xl text-navy-600/90">
              {line}
            </p>
          </Reveal>
        ))}
        ...
        <Reveal delay={0.15} y={16}>
          <p className="font-serif text-3xl sm:text-4xl lg:text-5xl text-navy-950">
            Clareza muda o jogo.
          </p>
        </Reveal>

        <Reveal delay={0.35} y={16}>
          <p className="mt-4 text-sm sm:text-base tracking-[0.14em] uppercase text-navy-500">
